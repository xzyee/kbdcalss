...
typedef struct _DEVICE_EXTENSION {
    ...
    //
    UNICODE_STRING  SymbolicLinkName;//使能或者不使能设备接口都靠他了

    //
    // Start of the class input data queue (really a circular buffer).
    //
    PKEYBOARD_INPUT_DATA InputData; //Ring Buffer就靠他了

    //
    // Insertion pointer for InputData.
    //
    PKEYBOARD_INPUT_DATA DataIn; //插入点位置

    //
    // Removal pointer for InputData.
    //
    PKEYBOARD_INPUT_DATA DataOut;//移出点位置

    //
    // Keyboard attributes.用内部IO控制查询键盘能支持什么，研究键盘从这个地方开始
    //
    KEYBOARD_ATTRIBUTES  KeyboardAttributes; //有键盘什么长度的重要信息：InputDataQueueLength

    //
    // A saved state of indicator lights
    //
    KEYBOARD_INDICATOR_PARAMETERS   IndicatorParameters; //键盘点灯，标准数据结构用标准的内部IO控制

    //
    // Spinlock used to synchronize access to the input data queue and its
    // insertion/removal pointers.
    //
    KSPIN_LOCK SpinLock;//Ring buffer和读IRP队列使用

    //
    // Queue of pended read requests sent to this port.  Access to this queue is
    // guarded by SpinLock
    //
    LIST_ENTRY ReadQueue; //要读键盘数据的IRP
    ...
    //
    DEVICE_POWER_STATE DeviceState; //设备功率状态，标准数据结构用标准的内部IO控制
    SYSTEM_POWER_STATE SystemState; //系统功率状态，标准数据结构用标准的内部IO控制
    ...
    //
    DEVICE_POWER_STATE MinDeviceWakeState; //唤醒设备，不是唤醒系统

    //
    // Lowest system state that the machine can be in and have the device wake it up
    //
    SYSTEM_POWER_STATE MinSystemWakeState; //唤醒系统，不是唤醒设备

    //
    // Actual wait wake irp
    //
    PIRP WaitWakeIrp; //实际要唤醒（设备或系统）的IRP，为何放在这里挂者，别跑了

    //
    // Duplicate wait wake irp getting completed because another was queued.
    //
    PIRP ExtraWaitWakeIrp; //干么再搞一个唤醒IRP？

    //
    // Target Device Notification Handle
    //
    PVOID TargetNotifyHandle; //注册回调函数给的存根，用于反注册

    //
    // Only used for a legacy port device
    //
    LIST_ENTRY Link; //登记在Globals.LegacyDeviceList，卸载要用，要是被挂就是说本设备是传统非PNP的
	//卸载其实很简单，把Ring Buffer内存释放（如果有的话），并且调用IoDeleteDevice即可
 

    //
    // Used only for a legacy port device when grand master mode is off
    //
    PFILE_OBJECT File; //为什么传统端口设备才用？答案在下面
	//因为有大师时，File保存在Globals.AssocClassList[tmp_deviceExtension->UnitId].File
...
typedef struct _GLOBALS {
    ...
    // also known as the all for one and one for all flag is set
    //
    ...
    //
    UNICODE_STRING      RegistryPath; //来自DriverEntry的第二个参数

    //
    // The base name for all class objects created as mice.
    //
    UNICODE_STRING      BaseClassName; //要不来自注册表,一般是"KeyboardClass"
    ...
    // them when we unload.
    //
    LIST_ENTRY LegacyDeviceList;//unload表，DriverUnload例程中卸载
} GLOBALS, *PGLOBALS;
...
//windows规定的回调函数
//为什么没有DRIVER_REINITIALIZE KeyboardClassFindMorePorts;这一句，不是要求的么？
